...
import chess.domain.InvitationStatuses.{Accepted, Pending, Canceled}
...
class CancelInvitationTest extends TestBase {
  ...
  behavior of "PUT /invitations/cancel"

  it should "cancel invitation correctly" in {
    //arrange
    ...
    //act
    val result = Rest.cancelInvitation(session.token, invitation.id).to[InvitationData]
    //assert
    val expected = invitation.copy(status = Canceled, completedAt = result.completedAt)
  ...
  it should "return 401 (Unauthorized) CREDENTIALS_REJECTED if token is incorrect" in {
    //act
    val result = Rest.cancelInvitation(Token.generate(), InvitationId.generate()).toErrorResult
    //assert
    result should be (Unauthorized.credentialsRejected)
  }

  it should "return 403 (Forbidden) ACCESS_DENIED if user is not inviter" in {
    //arrange
    val user, invitee = Mongo.addUser()
    val session = Mongo.addSession(userId = invitee.id)
    val invitation = Mongo.addInvitation(inviterId = user.id, inviteeId = invitee.id, status = Pending)
    //act
    val result = Rest.cancelInvitation(session.token, invitation.id).toErrorResult
    //assert
  ...
  it should "return 404 (Not Found) INVITATION_NOT_FOUND if invitation not found" in {
    ...
    val session = Mongo.addSession(userId = user.id)
    //act
    val result = Rest.cancelInvitation(session.token, InvitationId.generate()).toErrorResult
    //assert
